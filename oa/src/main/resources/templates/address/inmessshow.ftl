	...
	<table class="table" border=""  style="width: 90%;margin: auto;margin-bottom: 20px;">
		<tr>
			...
			<td rowspan="3" colspan="2">
				<div id="">
					<img src="images/user${(user.imgPath)!'timg.jpg'}" style="width: 120px;height: 120px;border-radius: 50%;margin-left: 15%;"/>
				</div>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>${user.deptId.deptName}</td>
			<td class="text-right">职位:</td>
			<td class="text-left">${(user.positionId.name)!'空的'}</td>
		</tr>
		<tr>
			<td class="text-right" >角色:</td>
			<td>${(user.roleId.roleName)!'空的'}</td>
			<td class="text-right">毕业学校:</td>
			<td class="text-left">${(user.userSchool)!'空的'}</td>
		</tr>
		<tr>
