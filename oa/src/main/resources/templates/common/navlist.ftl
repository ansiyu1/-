...
<#--				</#if>-->
		...
		<a href="#" class="green-none white" data-toggle="dropdown">
				<img src="/images/user${(user.imgPath)!'/timg.jpg'}" class="user-image" /> <span>${user.userName}</span>
		</a> <!--设置点击按钮弹出用户面板层-->
			<ul class="dropdown-menu" style="padding:0;">
				<li class="user-header green"><img src="/images/user${(user.imgPath)!'/timg.jpg'}"
					class="img-circle" style="width: 100px;height:100px;" />
					<p class="white" style="">
						<span>${dept.deptName} </span> / <span> ${role.roleName}</span><br> <small>${position.name}</small>
					</p></li>
