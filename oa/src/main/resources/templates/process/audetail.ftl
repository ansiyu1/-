	...
	<div class="col-md-12">
		...
		<form action="susave">
			...
			<div class="box-body">
				...
				<div class="shen">
					<p>${(list.poname)!''}：${(list.username)!''}<span class="pull-right">${(list.retime)?string('yyyy-MM-dd HH:mm:ss')!''}</span></p>
					<div >
				...
				<div class="shen">
					<p>申请人：${(process.userId.userName)!''}<span class="pull-right">${(process.applyTime)?string('yyyy-MM-dd hh:mm:ss')!''}</span></p>
					<div >
					<div style="display:inline-block;position:absolute;">
					...
					<p class="pa" style="padding-top: 9px;">开始时间：${(process.startTime)?string('yyyy-MM-dd HH:mm:ss')!''}</p>
					<p class="pa" >结束时间：${(process.endTime)?string('yyyy-MM-dd HH:mm:ss')!''}</p>
					<p class="pa" >申请理由：${(process.processDescribe)!''}</p>
					</#if>
					<#if typename=="转正申请">
					<p class="pa" style="padding-top: 9px;">试用/实习职位：${(position.position.name)!''}</p>
					<p class="pa" >开始时间：${(process.startTime)?string('yyyy-MM-dd HH:mm:ss')!''}</p>
					<p class="pa" >结束时间：${(process.endTime)?string('yyyy-MM-dd HH:mm:ss')!''}</p>
					</#if>
					<#if typename=="离职申请">
