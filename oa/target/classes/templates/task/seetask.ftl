						...
						<div class=" mailbox-read-info">
							...
							<#if task.reciverlist??>
								<h5 class="fonts">
									...
									<span id="ctl00_cphMain_lblDate" class="mailbox-read-time pull-right">${task.publishTime?string('yyyy-MM-dd HH-mm-ss') }</span>
								</h5>
								</#if>
							</div>
						<div class="mailbox-read-message">
							...
							<span id="ctl00_cphMain_lblFeedback">
								<h5 style="margin-top: 20px;">
									任务进度（${task.starTime?string('yyyy-MM-dd HH-mm-ss')}至 ${task.endTime?string('yyyy-MM-dd HH-mm-ss')}） <small
										class="pull-right">${status.sortPrecent}</small>
								</h5>
								<div class="progress xs" style="margin: 10px 0;">
									<div class="progress-bar progress-bar-aqua"
										style="width:${status.sortPrecent}" role="progressbar"
									...
									<div>
										${logger.username}
											<span style="font-size:8pt; color:#999; margin-left:3px;">${logger.createTime?string('yyyy HH:mm:ss')}</span>
